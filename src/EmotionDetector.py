...
def preprocessTestImage(image_path,testingParams):
    ...
    image_data = Image.open(image_path).convert('L') #grayscale
    image_data.load()  
    img_box = [0,0,48,48]
...
if __name__ == "__main__":
    ...
    if args.test_instances == 1:
        print(test_SingleInstance(args.saved_model, args.img_path))
    else:
        testSeveralInstances(args.saved_model, args.img_path)

# python -W ignore EmotionDetector.py -t 1 -m ../vgg13.model -p data/FER2013Test/fer0032220.png
